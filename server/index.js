...
app.post("/api/compile", async (req, res) => {
  ...
  try {
    ...
    const docker = getDockerStatus();
    if (docker.available) {
      const result = await runInDocker({ dockerBin: docker.bin, workDir, mode, standard });
...
async function runInDocker({ dockerBin, workDir, mode, standard }) {
  const mountPath = toDockerMountPath(dockerBin, workDir);
  const script = [
    "set -eu",
    `gcc -std=${standard} -Wall -Wextra -O0 -pipe -fno-diagnostics-color -o /sandbox/main /work/main.c`,
    mode === "run"
      ? `timeout -s KILL ${RUN_TIMEOUT_SECONDS}s /sandbox/main < /work/input.txt`
      : "true"
  ].join("\n");
  ...
  return runCommand(
    ...
    [
      ...
      "--tmpfs",
      "/sandbox:rw,nosuid,nodev,size=64m,mode=1777",
      "-v",
      `${mountPath}:/work:ro`,
      RUNNER_IMAGE,
      "/bin/sh",
